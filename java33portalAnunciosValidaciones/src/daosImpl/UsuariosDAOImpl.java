...
public class UsuariosDAOImpl extends GenericDAO implements UsuariosDAO{
	...
	@Override
	public void guardarCambiosUsuarios(Usuario usuario) {
		conectar();
		try {
			PreparedStatement ps = miConexion.prepareStatement(ConstantesSQL.GUARDAR_CAMBIOS_USUARIO);
			ps.setString(1, usuario.getNombre());
			ps.setString(2, usuario.getEmail());
			ps.setString(3, usuario.getPoblacion());
			ps.setString(4, usuario.getPass());
	...
	public Usuario obtenerUsuariosPorId(int id) {
		...
		try {
			PreparedStatement ps = miConexion.prepareStatement(ConstantesSQL.OBTENER_USUARIO_POR_ID);
			ps.setInt(1, id);
			ResultSet rs = ps.executeQuery();
			if(rs.next()){
